...
async function fetchHombreProducts() {
    try {
        ...
        if (rows.length > 0) {
            // Filtra las filas donde la columna "CATEGORIA" es "Hombre"
            const hombreProducts = rows.filter(product => product[2].toLowerCase() === 'hombre'); 
            renderHombreProducts(hombreProducts);
        } else {
...
function renderHombreProducts(products) {
...
// Llama a la función para obtener y mostrar solo los productos de "Hombre" al cargar la página
fetchHombreProducts();
